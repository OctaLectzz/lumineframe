    ...
    <q-form @submit="createData">
      <q-card style="min-width: 400px">
        <q-card-section class="row items-center q-py-sm">
          <div class="text-h6">{{ $t('dashboard.photo.createText') }}</div>
          <q-space />
...
const createData = async () => {
  ...
  try {
    ...
    toast.success(t('dashboard.photo.crud.successCreateMsg'))
    emits('added')
  } catch (error) {
    console.error('Error submitting form:', error)
    toast.error(t('dashboard.photo.crud.failedCreateMsg'))
  }
