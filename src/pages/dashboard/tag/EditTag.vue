    ...
    <q-form @submit="editData">
      <q-card style="min-width: 400px">
        <q-card-section class="row items-center q-py-sm">
          <div class="text-h6">{{ $t('dashboard.tag.editText') }}</div>
          <q-space />
    ...
    (v) => {
      if (typeof v === 'string') {
        if (tags.value) {
          ...
          return !tags.value.some((tag) => tag.name.toLowerCase() === v.toLowerCase() && tag.name.toLowerCase() !== tagName) || t('dashboard.tag.validate.nameUnique')
        }
      }
      return true
    }
...
const editData = async () => {
  ...
  try {
    ...
    toast.success(t('dashboard.tag.crud.successEditMsg'))
    emits('edited')
  } catch (error) {
    console.error('Error submitting form:', error)
    toast.error(t('dashboard.tag.crud.failedEditMsg'))
  }
